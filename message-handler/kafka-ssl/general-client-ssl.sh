...
keytool -genkey -keystore $name/$name.client.keystore.jks -validity 365 -storepass $password -keypass $password -keyalg RSA -dname "CN=$name" -storetype pkcs12
...
openssl x509 -req -CA ca-cert -CAkey ca-key -in $name/client-cert-sign-request -out $name/client-signed-cert -days 365 -CAcreateserial -passin pass:$password
