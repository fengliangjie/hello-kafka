...
password=123456
#服务端生成密钥库和证书
keytool -genkey -keystore kafka.server.keystore.jks -validity 3650 -storepass $password -keypass $password -keyalg RSA -dname "CN=$name" -storetype pkcs12
keytool -keystore kafka.server.keystore.jks -certreq -file cert-file -storepass $password -keypass $password

#配置CA，生成密钥和证书
openssl req -new -newkey rsa:4096 -days 3650 -x509 -subj "/CN=$name" -keyout ca-key -out ca-cert -nodes
#用CA的密钥和证书对服务端证书签名
